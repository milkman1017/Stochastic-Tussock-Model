...
int main() {
    ...
    double class_transition_matrix[9][9] = { 
        {0.0000,0.0063,0.0782,0.1730,0.2739,0.3202,0.4054,0.3779,0.3776},      //probability of a tiller of size class n transitioning to a class m
    ...
    outputFile << "TimeStep,SizeClass,Radius,X,Y,Z,Status\n";

    Tiller initial_tiller(1, 0.5, 0.001, 0, 0, 1); // initalize the first tiller at coords 0,0,0,
    ...
    for (int time_step = 0; time_step <= sim_time; time_step++) {
        ...
        for (Tiller& tiller : tillers) {
            ...
            if (surviveEvent < survival_matrix[0][size_class-1] / (0.5*distance)) {  //if tiller lives, determine new size class from transition probabilities
            ...
            //also to see what kind of relationship between distance and survival there is (linear, exponentional, etc)
                ...
                //will need to relate size class to reproduction
                //for now just slap in reproduction into every tiller to run and test the model
            ...
            step_data.emplace_back(tiller.getSizeClass(), tiller.getRadius(), tiller.getX(), tiller.getY(), tiller.getZ(), tiller.getStatus());
            } 
        ...
        for (Tiller& data : step_data) {
            outputFile << time_step << "," << data.getSizeClass() <<','<<data.getRadius() << ',' <<data.getX()<<','<<data.getY()<<','<<data.getZ() << ','<< data.getStatus()<<'\n';
        }
    }
