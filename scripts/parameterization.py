...
def positive_param(name: str) -> bool:
    return name in {"ks", "kr", "k_crowd_survival", "k_crowd_reproduction", "k_crowd_establishment", "base_establishment"}


def bounded01_param(name: str) -> bool:
    return name in {"c_space_survival", "c_space_reproduction", "c_space_establishment", "base_establishment"}
...
def determine_active_params(config: configparser.ConfigParser) -> list[str]:
    ...
    if str(mech.get("use_spatial_establishment", "false")).lower() == "true":
        active.append("c_space_establishment")
    ...
    # establishment baseline only matters if establishment stage exists conceptually
    active.append("base_establishment")
...
def load_combined_config(config_path: str, cli_sites=None):
    ...
    paths = PathSettings(
        param_file=os.path.abspath(config.get("Paths", "param_file", fallback=os.path.join("parameters", "parameters.txt"))),
...
def random_initial_parameters(active_params: list[str]) -> OrderedDict:
    ...
    for name in active_params:
        ...
        elif name in {"c_space_survival", "c_space_reproduction", "c_space_establishment", "base_establishment"}:
...
def initialize_random_parameter_file(param_file: str, active_params: list[str]) -> OrderedDict:
...
def write_parameter_snapshot(parameters: OrderedDict, site_outdir: str):
    write_parameter_file(parameters, os.path.join(site_outdir, "parameters.txt"))


def write_model_runtime_ini(project_root: str, combined_config_path: str, param_file_rel_from_root: str, constraint_year: int, alive_overflow_threshold: int):
    ini_path = os.path.join(project_root, "parameterization.ini")
    cp = configparser.ConfigParser()
    cp["Parameterization"] = {
        "config_file": str(combined_config_path),
...
def sample_random_params_around(base_params: OrderedDict, active_params: list[str], log10_span: float) -> OrderedDict:
    ...
    for k in active_params:
        ...
        elif positive_param(k) and k != "base_establishment":
...
def fixed_axis_limits_from_observed(training_diameters: np.ndarray, bins: int = 30) -> dict:
    ...
    xmin = float(np.min(obs)); xmax = float(np.max(obs)); span = xmax - xmin
...
def tussock_model(config: configparser.ConfigParser, output_mode: str, project_root: str):
    ...
    p = subprocess.Popen([exe], cwd=project_root, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
...
def wasserstein_distance_1d(x, y):
    ...
    n = x_sorted.size; m = y_sorted.size
    i = 0; j = 0; cdfx = 0.0; cdfy = 0.0
    prev = min(x_sorted[0], y_sorted[0]); w1 = 0.0
    while i < n or j < m:
...
def barrier_loss(fail_stats, constraint_year, min_alive):
    ...
    v_alive = []; v_over = []; v_missing = []; v_early_ext = []
    for s in fail_stats:
...
def read_sim_summaries(sim_outdir: str, num_sims: int) -> pd.DataFrame:
    ...
    if not os.path.isdir(summary_dir):
        return pd.DataFrame({"sim_id": np.arange(num_sims, dtype=int), "final_t": -1, "final_diameter": np.nan, "alive_y": 0, "rmax_y": np.inf, "overflow_t": -1, "extinct_t": -1, "missing_year": 1, "alive_final": 0, "LeafArea": np.nan})
    dfs = []
    ...
    if not dfs:
        return pd.DataFrame(columns=["sim_id", "final_t", "final_diameter", "alive_y", "rmax_y", "overflow_t", "extinct_t", "missing_year", "alive_final", "LeafArea"])
...
def write_population_results(sim_df: pd.DataFrame, iteration_label: int, out_csv_path: str, overgrown_radius_threshold: float):
...
def diameter_objective(config, iteration_label, training_data, frames_dir, axis_limits, constraints: ConstraintSettings, plotting: PlotSettings):
    num_sims = int(config.get("Tussock Model", "nsims"))
    ...
    ok = ((df["missing_year"].to_numpy(dtype=int) == 0) & (df["alive_y"].to_numpy(dtype=int) >= constraints.min_alive_tillers) & (df["overflow_t"].to_numpy(dtype=int) < 0))
    ...
    for row in df.itertuples(index=False):
        ...
        fail_stats.append({"alive_y": 0 if missing_year else int(row.alive_y), "overflow_t": overflow_t, "extinct_t": extinct_t, "missing_year": missing_year, "alive_final": int(getattr(row, "alive_final", 0)), "final_t": int(getattr(row, "final_t", -1))})
    fit_mask = ((df["missing_year"].to_numpy(dtype=int) == 0) & (df["overflow_t"].to_numpy(dtype=int) < 0))
    if constraints.require_survive_to_end_for_fit:
    ...
    bar = barrier_loss(fail_stats=fail_stats, constraint_year=constraints.constraint_year, min_alive=constraints.min_alive_tillers)
    ...
    loss = (float(fit_loss) + 2.0 * obs_std * float(sd_loss) + 5.0 * obs_std * float(bar) + 2.0 * obs_std * low_alive_frac + 10.0 * obs_std * over_frac + 10.0 * obs_std * missing_frac + ext_term + 10.0 * obs_std * leaf_bad_frac + 5.0 * obs_std * pass_shortfall)
    ...
    if do_plot:
        ...
        ax.set_xlim(*axis_limits["xlim"]); ax.set_ylim(*axis_limits["ylim"]); ax.legend()
        ax.set_title(f"Iter: {iteration_label} | loss={loss:.3g} | pass={pass_count}/{num_sims} | fit={fit_loss:.3g} | sd={sd_loss:.3g} | bar={bar:.3g} | overflow={over_frac:.1%} | extinct_final={extinct_final}/{num_sims}")
        ax.set_xlabel("Tussock Diameter")
        plt.savefig(os.path.join(frames_dir, f"Mean_Tuss_diameter_iteration_{iteration_label}.png"), dpi=200)
        plt.close(fig)
    return float(loss)
...
def write_optimization_results(parameters: OrderedDict, active_params: list[str], loss: float, iteration_label, out_csv_path: str):
    _safe_makedirs(os.path.dirname(out_csv_path))
    file_exists = os.path.exists(out_csv_path)
...
def cma_es_optimize(f, x0, sigma0, max_evals, tol_f, tol_x, project_fn, popsize=0, patience=20):
    ...
    m = project_fn(np.array(x0, dtype=float)); sigma = float(max(1e-12, sigma0))
    C = np.eye(n); B = np.eye(n); D = np.ones(n); invsqrtC = np.eye(n); pc = np.zeros(n); ps = np.zeros(n)
    evals = 0; generation = 0; best_x = m.copy(); best_f = float("inf"); stall = 0
    while evals < max_evals:
        ...
        for k in range(cur_lambda):
            fvals[k] = f(arx[k]); evals += 1
            if fvals[k] < best_f:
                best_f = float(fvals[k]); best_x = arx[k].copy()
        order = np.argsort(fvals); arx = arx[order]; ary = ary[order]; fvals = fvals[order]
        use_mu = min(mu, cur_lambda); w = weights[:use_mu]; w = w / np.sum(w)
        ...
        sigma = sigma * np.exp((cs / damps) * (norm_ps / chi_n - 1.0)); sigma = float(max(1e-12, sigma))
        C = 0.5 * (C + C.T)
        eigvals, eigvecs = np.linalg.eigh(C + 1e-12 * np.eye(n))
        eigvals = np.maximum(eigvals, 1e-20); D = np.sqrt(eigvals); B = eigvecs; invsqrtC = B @ np.diag(1.0 / D) @ B.T
...
def build_projection_functions(opt_settings: OptimizationSettings, active_params: list[str], template_params: OrderedDict):
    ...
    positive_idxs = {name: i for i, name in enumerate(active_params) if positive_param(name) and name != "base_establishment"}
...
def main():
    ...
    if not os.path.exists(run_settings.paths.param_file):
        ...
        template_params = initialize_random_parameter_file(run_settings.paths.param_file, run_settings.active_params)
    else:
    ...
    project_vec, x_to_model_params, params_to_x = build_projection_functions(run_settings.optimization, run_settings.active_params, template_params)
    ...
    for site in site_list:
        ...
        print("====================================\n")
        if site == "ALL":
            training_data = full_training_df.copy(); site_tag = "ALL"
        else:
            training_data = full_training_df[full_training_df["site"] == site].copy(); site_tag = str(site)
        training_data["field_davg"] = pd.to_numeric(training_data["diam"], errors="coerce")
        training_diameters = training_data["field_davg"].dropna().values
        axis_limits = fixed_axis_limits_from_observed(training_diameters, bins=30)
        site_outdir = os.path.join(run_settings.paths.output_dir, site_tag); os.makedirs(site_outdir, exist_ok=True)
        cpp_outdir = os.path.join(site_outdir, "simulation_outputs"); os.makedirs(cpp_outdir, exist_ok=True)
        ...
        pop_csv_path = os.path.join(site_outdir, "population_results.csv")
        for p in (opt_csv_path, pop_csv_path):
            if os.path.exists(p):
                os.remove(p)
        eval_label = 0; frame_labels = []; best_seen = {"loss": float("inf"), "params": None}
        ...
        def objective_vec(x: np.ndarray) -> float:
            ...
            write_model_runtime_ini(project_root, os.path.abspath(args.config), param_file_rel, run_settings.constraints.constraint_year, run_settings.constraints.alive_overflow_threshold)
            tussock_model(combined_config, output_mode="summary", project_root=project_root)
            loss = diameter_objective(combined_config, eval_label, training_data, frames_dir, axis_limits, run_settings.constraints, run_settings.plotting)
            ...
            sim_df = read_sim_summaries(cpp_outdir, num_sims=int(combined_config.get("Tussock Model", "nsims")))
            ...
            if loss < best_seen["loss"]:
                best_seen["loss"] = loss; best_seen["params"] = OrderedDict(params)
            return loss
        ...
        if len(run_settings.active_params) == 0:
            ...
            best_init_loss = float("inf"); best_init_params = OrderedDict(template_params)
            for _ in range(run_settings.optimization.n_init):
                trial_params = sample_random_params_around(template_params, run_settings.active_params, run_settings.optimization.init_log10_span)
                x_trial = project_vec(params_to_x(trial_params))
                loss_trial = objective_vec(x_trial)
                if loss_trial < best_init_loss:
                    best_init_loss = loss_trial; best_init_params = OrderedDict(trial_params)
            ...
            if run_settings.optimization.optimize_log_space:
                ...
                for i in range(x0.size):
                    mag = abs(x0[i])
                    step_scales.append(run_settings.optimization.step_frac * mag if mag > 1e-12 else run_settings.optimization.step_abs)
            ...
            if remaining_budget > 0 and x0.size > 0:
                best_x, _, _ = cma_es_optimize(objective_vec, x0, sigma0, remaining_budget, run_settings.optimization.tol_f, run_settings.optimization.tol_x, project_vec, run_settings.optimization.cma_popsize, run_settings.optimization.cma_patience)
        ...
        write_model_runtime_ini(project_root, os.path.abspath(args.config), param_file_rel, run_settings.constraints.constraint_year, run_settings.constraints.alive_overflow_threshold)
        final_sims_dir = os.path.join(site_outdir, "final_sims"); os.makedirs(final_sims_dir, exist_ok=True)
        ...
        final_summary_df = read_sim_summaries(final_sims_dir, num_sims=int(combined_config.get("Tussock Model", "nsims")))
        final_pop_csv_path = os.path.join(site_outdir, "final_population_results.csv")
        if os.path.exists(final_pop_csv_path):
            os.remove(final_pop_csv_path)
        ...
        if run_settings.plotting.plot_every and run_settings.plotting.plot_every > 0:
            animate_fitting(frames_dir, frame_labels, os.path.join(site_outdir, "diameter_dist_fitting.gif"))
